...
public class KafkaConnectS2IAssemblyOperatorTest {
    ...
    public void testUpdateCluster(TestContext context) {
        ...
        ConfigMap metricsCm = new ConfigMapBuilder().withNewMetadata()
                    .withName(KafkaConnectS2ICluster.metricConfigsName(clusterCmName))
                    .withNamespace(clusterCmNamespace)
                .endMetadata()
                .withData(Collections.singletonMap(AbstractModel.METRICS_CONFIG_FILE, METRICS_CONFIG))
                .build();
        when(mockCmOps.get(clusterCmNamespace, KafkaConnectS2ICluster.metricConfigsName(clusterCmName))).thenReturn(metricsCm);
